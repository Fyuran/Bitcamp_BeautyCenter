...
public class SetupFirstAccountFrame extends JFrame {
	...
	private static void createEmployee() {
		...
		cred = UserCredentialsDAO.insertUserCredentials(cred).orElseGet(() -> UserCredentialsDAO.getUserCredentials(username).get()); //if usercredentials already exist get that one
	...
	private static boolean validateInputs() {
		try {
			...
			!inputValidator.validateIban(txfIban.getText(), true) &&
			...
			!inputValidator.validateAlphanumeric(txfUsername.getText()) &&
			!inputValidator.validatePassword(txfPassword.getPassword()) &&
			!inputValidator.validateAlphanumeric(txfBirthplace.getText())) throw new RuntimeException("Dati non validi");

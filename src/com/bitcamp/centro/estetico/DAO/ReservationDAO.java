...
public class ReservationDAO {
	...
	public Map<Integer, Reservation> getAll() throws Exception {
		String query = "SELECT c.id as customer_id, c.name AS customer_name, c.surname AS customer_surname, "
				+ "r.id as reservation_id, r.date, r.state, e.id AS employee_id, e.name AS employee_name, "
		...
		try (PreparedStatement pstmt = connection.prepareStatement(query)) {
			...
			try (ResultSet rs = pstmt.executeQuery()) {
				while (rs.next()) {
					...
					reservation.setState(ReservationState.valueOf(rs.getString("state")));
	...
	public Reservation getReservation(int reservationId)throws Exception  {
		String query = "SELECT c.id as customer_id, c.name AS customer_name, c.surname AS customer_surname, "
				+ "r.id as reservation_id, r.date, r.state, "
		...
		try (PreparedStatement pstmt = connection.prepareStatement(query)) {
			...
			try (ResultSet rs = pstmt.executeQuery()) {
				while (rs.next()) {
					...
					ReservationState state = ReservationState.valueOf(stateStr);
	...
	public void insert(Reservation reservation) throws Exception{
		...
		JOptionPane.showMessageDialog(null, reservation.getDateTime());
		try (PreparedStatement pstmt = connection.prepareStatement(query)) {

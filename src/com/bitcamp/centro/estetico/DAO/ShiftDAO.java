...
public class ShiftDAO {
	...
	public Map<Integer, ShiftEmployee> getShifts() throws Exception {
		String query = "SELECT se.id AS shiftEmployee_id, s.start, s.end, s.type, s.id AS shift_id, e.name, e.surname, "
				...
				+ "ON se.shift_id = s.id JOIN beauty_centerdb.employee e ON se.employee_id = e.id WHERE s.is_enabled = ?;";
		...
		try (PreparedStatement pstmt = connection.prepareStatement(query)) {
			...
			try (ResultSet rs = pstmt.executeQuery()) {
				while (rs.next()) {
					...
					ShiftType type = ShiftType.valueOf(typeStr);
					
					Shift shift = new Shift(rs.getInt("shift_id"), rs.getTimestamp("start").toLocalDateTime(),
							rs.getTimestamp("end").toLocalDateTime(), type,
							rs.getString("notes"));
	...
	public static List<Shift> loadShiftsForEmployeeId(int employeeId) {
		List<Shift> shifts = new ArrayList<>();

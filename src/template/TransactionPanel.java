...
public class TransactionPanel extends JPanel {
	...
	private static Choice choicePaymentMethod;
	private static Choice choiceVAT;
	...
	public TransactionPanel(JFrame frame) {
		setLayout(null);
		setSize(1040, 900);
		parent = frame;
		...
		btnInsert.addActionListener(e -> {
			try {
				...
				VAT vat = VATDao.getAllVAT().get(choiceVAT.getSelectedIndex());
				...
				TransactionDAO.insertTransaction(new Transaction(price, paymentMethod, dateTime, customer, vat,
						BeautyCenterDAO.getBeautyCenter(1).get(), services));
				lbOutput.setText("Transazione inserita");
				fillRows();
			} catch (Exception ex) {
				JOptionPane.showMessageDialog(new JFrame(), "Dati errati o mancanti", "Errore di inserimento",
		...
		btnUpdate.addActionListener(e -> {
			try {
				...
				JOptionPane.showMessageDialog(new JFrame(), "Dati errati o mancanti", "Errore di aggiornamento",
						JOptionPane.ERROR_MESSAGE);
			}
		...
		choicePaymentMethod = new Choice();
		springLayoutActions.putConstraint(SpringLayout.NORTH, choicePaymentMethod, 85, SpringLayout.NORTH,
				actionsPanel);
		Object[] payMethods = PayMethod.class.getEnumConstants();
		for (Object constant : payMethods) { // add every translated constant to choice menu
			choicePaymentMethod.add(constant.toString());
		}
		...
		springLayoutActions.putConstraint(SpringLayout.NORTH, lblMetodoDiPagamento, -4, SpringLayout.NORTH,
				choicePaymentMethod);
		...
		springLayoutActions.putConstraint(SpringLayout.WEST, priceTextField, 0, SpringLayout.WEST, choicePaymentMethod);
		springLayoutActions.putConstraint(SpringLayout.EAST, priceTextField, 0, SpringLayout.EAST, choicePaymentMethod);
		...
		choiceVAT = new Choice();
		springLayoutActions.putConstraint(SpringLayout.NORTH, choiceVAT, 6, SpringLayout.SOUTH, choicePaymentMethod);
		springLayoutActions.putConstraint(SpringLayout.WEST, choiceVAT, 0, SpringLayout.WEST, choicePaymentMethod);
		for (VAT vat : VATDao.getAllVAT()) {
			choiceVAT.add(vat.toString());
		}
		...
		springLayoutActions.putConstraint(SpringLayout.EAST, choiceVAT, -240, SpringLayout.WEST,

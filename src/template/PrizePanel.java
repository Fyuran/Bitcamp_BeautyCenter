...
import com.centro.estetico.bitcamp.PrizeType;
...
public class PrizePanel extends JPanel {
	...
	public PrizePanel() {
		...
		btnDisable.setIcon(new ImageIcon(gestionePremi.class.getResource("/iconeGestionale/disattivo.png")));
	...
	private void loadActivePrizesToTable() {
	    tableModel.setRowCount(0);  // Resetta la tabella
	...
	private boolean validateInputs() {
	    // Validazione del nome premio
	    if (!inputValidator.validateAlphanumeric(txfNomePremio.getText(), "Nome Premio")) {
	        JOptionPane.showMessageDialog(null, "Il nome del premio non è valido. Deve contenere solo caratteri alfanumerici.");
	        return false;
	    }

	    // Validazione dell'importo premio
	    try {
	        double importo = Double.parseDouble(txfImportoPremio.getText());
	        if (!inputValidator.validateDecimal(importo, 0d, 10000d)) { // Valore massimo arbitrario per evitare errori
	            JOptionPane.showMessageDialog(null, "L'importo non è valido. Deve essere un numero compreso tra 0 e 10.000.");
	            return false;
	        }
	    } catch (NumberFormatException e) {
	        JOptionPane.showMessageDialog(null, "L'importo non è valido. Deve essere un numero.");
	        return false;
	    }
